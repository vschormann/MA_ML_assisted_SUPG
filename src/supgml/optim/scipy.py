...
"""Adapters for optimizing PyTorch modules with SciPy."""
...
class PyTorch_SciPy_interface():
    ...
    def _set_params(self, x):
        ...
        for p in self.nn.parameters():
            p.grad = None
            numel = p.numel()
            values = torch.as_tensor(x[pointer:pointer+numel], dtype=p.dtype, device=p.device)
            with torch.no_grad():
                p.copy_(values.view_as(p))
            pointer += numel
...
TorchScipyOptimizer = PyTorch_SciPy_interface
